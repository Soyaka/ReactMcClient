...
export default function CoursePageContainer() {
  ...
  return (
    <div className="flex flex-row px-[5rem] items-center justify-center  gap-10 2xl:gap-32 2xl:px-[10rem] py-[3rem]">
      <div className="flex flex-col gap-8 2xl:gap-12 ">
        ...
        <div className="relative left-2 flex flex-row gap-4 2xl:gap-6 ">
          <button className="relative top-6 bg-blue-500 text-white px-4 py-2 tracking-wider rounded-full cursor-pointer">
      ...
      <div className="content-pages flex flex-col gap-8 p-4 2xl:gap-12">
        ...
        <div className=" relative left-4 flex flex-col gap-3 2xl:gap-4">
          {course.pages.map((page, index) => (
            <div key={page.id} className="text-lg underline cursor-pointer" onClick={() => handlePageClick(page)}>
              {page.title}
            </div>
          ))}
        </div>
...
// TODO: Define the types for course and page objects
type Page = {
  id: string;
  title: string;
  sections: {
